...
MarketingAreaWorkerHomePageController is a controller for the Marketing Area Worker home page GUI.

It handles the interactions between the GUI elements and the business logic.

*/

public class MarketingAreaWorkerHomePageController implements Initializable {
	...
	@FXML
	private Button ActivateSalesbtn;
     ...
     * If the user is in OL Marketing Area Manager, opens the Online Store New Order page.
     * If the user is in EK Marketing Area Manager, opens the Create New Order page.
     *
    ...
    /**
     * Handles the press of the Activate Sales button.
     * Hides the current window and opens the Activate Sales page for Marketing Area Worker.
     *
     * @param event the press of the Activate Sales button
     * @throws Exception
     */
    @FXML
    void pressActivateSalesbtn(ActionEvent event) throws Exception {
        ((Node) event.getSource()).getScene().getWindow().hide(); // hiding primary window
        Stage primaryStage = new Stage();
        primaryStage.initStyle(StageStyle.UNDECORATED);
        MarketingAreaWorkerActivateSalesController mawasc = new MarketingAreaWorkerActivateSalesController();
        MarketingAreaWorkerActivateSalesController.ekOrOl(ekOrOLFromConnect);
        MarketingAreaWorkerActivateSalesController.getUserData(arrFromServerRet);
        mawasc.start(primaryStage);
    }
    ...
    /**
     * Opens the Marketing Area Worker home page GUI.
     *
	...
	public void start(Stage primaryStage) throws Exception {
		...
		AnchorPane root = FXMLLoader.load(getClass().getResource("/gui/MarketingAreaWorkerHomePage.fxml"));
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		if (ekOrOLFromConnect.get(0).equals("EK")) {
			ActivateSalesbtn.setDisable(true);
